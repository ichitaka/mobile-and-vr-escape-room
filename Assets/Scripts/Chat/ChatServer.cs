...
public class ChatServer : MonoBehaviour
{
	...
	void OnGUI()
	{
		if (server == null)
		{
			if (GUI.Button(new Rect(110, 320, 200, 20), "Start Chat Server (s)") || Input.GetKeyDown(KeyCode.S))
			{
				Setup();
			}
		}
		...
		{
			if (GUI.Button(new Rect(110, 320, 200, 20), "Stop Chat Server (s)") || Input.GetKeyDown(KeyCode.S))
			{
				server.Stop();
				server = null;
			}
